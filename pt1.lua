...
Fish1:AddToggle({
    ...
    Callback = function(enabled)
        if enabled then
            ...
            NameLabel.Text = "CHLOEX PANEL FISHING"
...
local EnchantStonePanel = SellingSection:AddParagraph({
    Name = "Enchant Stone Left Status",
    Content = "Counting...",
})
...
local TotemDropdown = TotemSection:AddDropdown({
    Name = "Select Totem to Auto Place",
...
Panel:AddToggle({
    ...
    Callback = function(enabled)
        ...
        if enabled then
            ...
            local NameLabel = Instance.new("TextLabel", mainCard)
            NameLabel.Size = UDim2.new(1, -20, 0, 36)
            NameLabel.Position = UDim2.new(0, 10, 0, 8)
            NameLabel.BackgroundTransparency = 1
            NameLabel.Font = Enum.Font.GothamBold
            NameLabel.Text = "CHLOEX KAITUN PANEL"
            NameLabel.TextSize = 22
            NameLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
            ...
            local function createPanelSection(Name, height)
                local NameLabel = Instance.new("TextLabel", scrollingFrame)
...
ClassicEventSection:AddButton({
    Name = "Teleport to Classic Event",
    Callback = function()
        local character = Settings.player.Character or Settings.player.CharacterAdded:Wait()
        local humanoidRootPart = character:FindFirstChild("HumanoidRootPart")
        if humanoidRootPart then
...
WebhookSection:AddDropdown({
    Name = "Select Fish Names to Notify",
    Options = FishNames,
    Multi = true,
    Callback = function(selected)
