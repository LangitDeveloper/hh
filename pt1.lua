...
local IsengSection = PlayerTab:Section({Title = "Iseng Features"})

local IsengV1Toggle = IsengSection:Toggle({
    Title = "V1: Teleport All Players",
        ...
        if value then
            task.spawn(function()
                while IsengV1_Enabled do
                    ...
                    local ourRoot = ourCharacter and ourCharacter:FindFirstChild("HumanoidRootPart")
...
IsengSection:Divider()

local PlayerListForV2 = {}

local function UpdatePlayerListForV2()
    PlayerListForV2 = {}
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            table.insert(PlayerListForV2, player.Name)
        end
    end
end

UpdatePlayerListForV2()

local VictimDropdown = IsengSection:Dropdown({
    Title = "Select Victim (V2)",
    Values = PlayerListForV2,
    Callback = function(value)
...
IsengSection:Button({
    ...
    Callback = function()
        UpdatePlayerListForV2()
        VictimDropdown:Refresh(PlayerListForV2)
        MahiruUi:Notify({
            Title = "Iseng V2",
...
local IsengV2Toggle = IsengSection:Toggle({
    Title = "V2: Fly & Explode",
    Desc = "Player yang dipilih akan terbang lalu meledak",
    Value = false,
    Callback = function(value)
        IsengV2_Enabled = value
        
        if value and SelectedVictim then
            task.spawn(function()
                
                local victim = Players:FindFirstChild(SelectedVictim)
                if not victim or not victim.Character then
                    IsengV2Toggle:Set(false)
                    return
                end
                ...
                MahiruUi:Notify({
                    Title = "Iseng V2",
                    Content = SelectedVictim .. " terbang!",
                    Duration = 2,
                    Icon = "rocket",
                })
                ...
                if IsengV2_Enabled then
                    ...
                    MahiruUi:Notify({
                        Title = "Iseng V2",
                        Content = SelectedVictim .. " meledak!",
                        Duration = 2,
                        Icon = "bomb",
                    })
        ...
        elseif value and not SelectedVictim then
            MahiruUi:Notify({
                Title = "Error",
...
local PlayerListForV3 = {}

local function UpdatePlayerListForV3()
    PlayerListForV3 = {}
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            table.insert(PlayerListForV3, player.Name)
        end
    end
end

UpdatePlayerListForV3()

local FreezeVictimDropdown = IsengSection:Dropdown({
    Title = "Select Victim (V3)",
    Values = PlayerListForV3,
    Callback = function(value)
        SelectedVictim = value
    end,
})
...
IsengSection:Button({
    Title = "Refresh Player List (V3)",
    Callback = function()
        UpdatePlayerListForV3()
        FreezeVictimDropdown:Refresh(PlayerListForV3)
        MahiruUi:Notify({
            Title = "Iseng V3",
            Content = "Player list refreshed!",
            Duration = 2,
...
local IsengV3Toggle = IsengSection:Toggle({
    Title = "V3: Freeze Player",
    Desc = "Player yang dipilih akan freeze/lumpuh",
    Value = false,
    Callback = function(value)
        IsengV3_Enabled = value
        
        if value and SelectedVictim then
            task.spawn(function()
                local victim = Players:FindFirstChild(SelectedVictim)
                if not victim then
                    IsengV3Toggle:Set(false)
                    return
                end
                ...
                local function UnfreezePlayer(player)
                    ...
                    if data then
                        ...
                        if char then
                            ...
                            local highlight = char:FindFirstChild("Mahiru_Freeze")
                            if highlight then
                                highlight:Destroy()
                            end
                        end
                ...
                MahiruUi:Notify({
                    Title = "Iseng V3",
                    Content = SelectedVictim .. " telah di-freeze!",
                    Duration = 2,
                    Icon = "snowflake",
                })
                
                
                while IsengV3_Enabled and SelectedVictim == victim.Name do
                  
                    if not FrozenPlayers[victim] and victim.Character then
                        FreezePlayer(victim)
                    end
                    task.wait(0.5)
                end
                ...
                MahiruUi:Notify({
                    Title = "Iseng V3",
                    Content = SelectedVictim .. " telah di-unfreeze!",
                    Duration = 2,
                    Icon = "sun",
                })
            end)
...
IsengSection:Button({
    ...
    Desc = "Lepaskan semua player yang di-freeze",
    Callback = function()
        for player, _ in pairs(FrozenPlayers) do
            local data = FrozenPlayers[player]
            if data then
        ...
        MahiruUi:Notify({
            Title = "Iseng V3",
            Content = "Semua player telah di-unfreeze!",
            Duration = 2,
            Icon = "sun",
        })
    end,
...
ConfigManager:Register("isengV2Toggle", IsengV2Toggle)
ConfigManager:Register("isengV3Toggle", IsengV3Toggle)
...
Window:OnDestroy(function()
    ...
    if IsengV2Toggle then IsengV2Toggle:Set(false) end
    if IsengV3Toggle then IsengV3Toggle:Set(false) end
