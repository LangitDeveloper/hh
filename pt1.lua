...
local PlayerList = {}
-- Tambahkan variabel debug di global variables
...
local function AddDebugLog(message, color)
    ...
    local time = string.format("[%.2f]", tick() - LastActionTime)
    table.insert(DebugLogs, 1, {
        Time = os.date("%H:%M:%S"),
        Delta = time,
        Message = message,
    ...
    if #DebugLogs > MaxLogs then
        table.remove(DebugLogs, MaxLogs + 1)
    end
...
-- Fungsi untuk membuat GUI debug
local function CreateDebugGui()
    ...
    title.Text = "🐟 MAHIRU FISHING DEBUGGER 🐟"
    ...
    statsText.Text = "Status: Waiting for actions..."
    ...
    logsScrolling.Name = "LogsScrolling"
    logsScrolling.Parent = mainFrame
    logsScrolling.Size = UDim2.new(1, -20, 1, -100)
    logsScrolling.Position = UDim2.new(0, 10, 0, 90)
    logsScrolling.BackgroundColor3 = Color3.fromRGB(15, 15, 25)
    logsScrolling.BackgroundTransparency = 0.2
    logsScrolling.BorderSizePixel = 0
    logsScrolling.ScrollBarThickness = 8
    logsScrolling.AutomaticCanvasSize = Enum.AutomaticSize.Y
    ...
    clearBtn.Position = UDim2.new(0.5, -50, 1, -35)
...
function UpdateDebugGui()
    if not DebugGui or not DebugEnabled then return end
    
    -- Update stats
    local activeFishing = IsLegitFishing or IsInstantFishing or IsBlatantFishing
    local statusText = string.format("Status: %s | Logs: %d", 
        activeFishing and "ACTIVE FISHING" or "IDLE",
        #DebugLogs
    )
    
    DebugGui.StatsText.Text = statusText
    
    -- Clear existing logs
    for _, child in ipairs(DebugGui.LogsScrolling:GetChildren()) do
    ...
    for i, log in ipairs(DebugLogs) do
        ...
        logFrame.Parent = DebugGui.LogsScrolling
...
-- Hook untuk menangkap semua remote fishing secara otomatis
local function HookFishingRemotes()
    -- Hook untuk RF_FishCaught
    local originalRECaught = Remotes.RE_FishCaught.FireServer
    Remotes.RE_FishCaught.FireServer = function(self, ...)
        local args = {...}
        AddDebugLog(string.format("📥 RE_FishCaught called (Args: %d)", #args), 
                   Color3.fromRGB(255, 150, 255))
        return originalRECaught(self, ...)
    end
    
    -- Hook untuk RF_Fishing
    local originalRFFishing = Remotes.RF_Fishing.FireServer
    Remotes.RF_Fishing.FireServer = function(self, ...)
        local args = {...}
        AddDebugLog(string.format("🎣 RF_Fishing called (Args: %d)", #args), 
                   Color3.fromRGB(100, 255, 200))
        return originalRFFishing(self, ...)
    end
    
    -- Hook untuk RF_Minigame
    local originalRFMinigame = Remotes.RF_Minigame.InvokeServer
    Remotes.RF_Minigame.InvokeServer = function(self, ...)
        local args = {...}
        AddDebugLog(string.format("🎮 RF_Minigame called (Args: %d)", #args), 
                   Color3.fromRGB(200, 255, 100))
        return originalRFMinigame(self, ...)
    end
    
    AddDebugLog("✅ Fishing remotes hooked for monitoring", Color3.fromRGB(100, 255, 100))
end
...
function StartBlatantFishingDebug()
    IsBlatantFishing = true
    LastActionTime = tick()
    
    -- Enable debug jika belum
    if not DebugEnabled then
        DebugEnabled = true
        CreateDebugGui()
        AddDebugLog("🔧 Debug mode activated", Color3.fromRGB(255, 255, 100))
    end
    
    AddDebugLog("🎣 STARTING BLATANT FISHING DEBUG", Color3.fromRGB(100, 255, 100))
    
    Remotes.RF_AutoFishing:InvokeServer(true)
    AddDebugLog("✓ AutoFishing: true", Color3.fromRGB(100, 200, 255))
    ...
    task.spawn(function() 
        while IsBlatantFishing do
            ...
            task.spawn(function()
                -- Step 1: Charge
                AddDebugLog("→ Step 1: Charging...", Color3.fromRGB(255, 200, 100))
                local chargeSuccess, chargeResult = pcall(function()
                    return Remotes.RF_Charge:InvokeServer(workspace:GetServerTimeNow())
                end)
                
                if chargeSuccess then
                    AddDebugLog("✓ RF_Charge success", Color3.fromRGB(100, 255, 100))
                    if type(chargeResult) == "table" then
                        AddDebugLog("  Result: " .. tostring(chargeResult), Color3.fromRGB(150, 255, 150))
                    end
                else
                    AddDebugLog("✗ RF_Charge failed", Color3.fromRGB(255, 100, 100))
                end
                
                task.wait(0.1)
                
                -- Step 2: Minigame
                AddDebugLog("→ Step 2: Minigame...", Color3.fromRGB(255, 200, 100))
                local minigameSuccess, minigameResult = pcall(function()
                    return Remotes.RF_Minigame:InvokeServer(-1, 0.999, 100)
                end)
                
                if minigameSuccess then
                    AddDebugLog("✓ RF_Minigame success", Color3.fromRGB(100, 255, 100))
                else
                    AddDebugLog("✗ RF_Minigame failed", Color3.fromRGB(255, 100, 100))
                end
                
                -- Step 3: Try different complete methods
                AddDebugLog("→ Step 3: Completing...", Color3.fromRGB(255, 200, 100))
                
                -- Method A: RF_CatchFishCompleted
                local methodASuccess = pcall(function()
                    Remotes.RF_CatchFishCompleted:FireServer()
                end)
                
                if methodASuccess then
                    AddDebugLog("✓ RF_CatchFishCompleted (FireServer)", Color3.fromRGB(100, 255, 100))
                else
                    AddDebugLog("✗ RF_CatchFishCompleted (FireServer)", Color3.fromRGB(255, 150, 100))
                    
                    -- Try InvokeServer
                    local methodA2Success = pcall(function()
                        Remotes.RF_CatchFishCompleted:InvokeServer()
                    end)
                    
                    if methodA2Success then
                        AddDebugLog("✓ RF_CatchFishCompleted (InvokeServer)", Color3.fromRGB(100, 255, 100))
                    end
                end
                
                task.wait(0.05)
                
                -- Method B: RF_Fishing (old method)
                local methodBSuccess = pcall(function()
                    Remotes.RF_Fishing:FireServer()
                end)
                
                if methodBSuccess then
                    AddDebugLog("✓ RF_Fishing (FireServer)", Color3.fromRGB(150, 200, 255))
                end
                
                task.wait(0.05)
                
                -- Method C: RF_Cancel (if needed)
                local methodCSuccess = pcall(function()
                    Remotes.RF_Cancel:InvokeServer()
                end)
                
                if methodCSuccess then
                    AddDebugLog("✓ RF_Cancel called", Color3.fromRGB(200, 150, 255))
                end
                
                -- Check fish count
                local newCount = GetFishCount()
                if newCount > CurrentFishCount then
                    AddDebugLog(string.format("🎉 FISH CAUGHT! (+%d)", newCount - CurrentFishCount), 
                              Color3.fromRGB(255, 215, 0))
                    CurrentFishCount = newCount
                end
                
                AddDebugLog("✓ Iteration complete", Color3.fromRGB(100, 255, 255))
            end)
            
            local waitTime = BlatantReelDelay or 1.9
            AddDebugLog(string.format("⏳ Waiting %.1fs for next cast...", waitTime), 
                       Color3.fromRGB(200, 200, 200))
            task.wait(waitTime)
        end
        
        AddDebugLog("🛑 Fishing stopped", Color3.fromRGB(255, 100, 100))
    end)
end
...
-- Di bagian UtilitiesTab atau buat tab baru
local DebugSection = UtilitiesTab:Section({Title = "Debug & Monitor"})

DebugSection:Toggle({
    Title = "Enable Debug Mode",
    Desc = "Show real-time fishing debugger",
    Value = false,
    Callback = function(value)
        DebugEnabled = value
        ...
        if value then
            CreateDebugGui()
            HookFishingRemotes()
            AddDebugLog("🔧 Mahiru Debugger Activated", Color3.fromRGB(255, 255, 100))
            AddDebugLog("👉 Start fishing to see remote calls", Color3.fromRGB(200, 200, 255))
        else
            if DebugGui and DebugGui.ScreenGui then
                DebugGui.ScreenGui:Destroy()
                DebugGui = nil
            end
        end
    end,
...
DebugSection:Toggle({
    Title = "Auto-Hook Remotes",
    Desc = "Automatically hook all fishing remotes",
    Value = false,
    Callback = function(value)
        if value then
            HookFishingRemotes()
...
DebugSection:Button({
    Title = "Test Manual Fishing",
    Desc = "Trigger fishing manually to see remotes",
    Callback = function()
        if not DebugEnabled then
            DebugEnabled = true
            CreateDebugGui()
        ...
        AddDebugLog("🧪 MANUAL FISHING TEST", Color3.fromRGB(255, 200, 100))
            ...
            if name:find("Fish") or name:find("Fishing") then
                pcall(function()
                    if remote:IsA("RemoteEvent") then
                        remote:FireServer()
                        AddDebugLog(string.format("🔥 %s: FireServer()", name), 
                                   Color3.fromRGB(200, 150, 255))
                    elseif remote:IsA("RemoteFunction") then
                        remote:InvokeServer()
                        AddDebugLog(string.format("📞 %s: InvokeServer()", name), 
...
-- Di FishingTab, tambahkan versi debug
FishingTab:Section({Title = "Blatant Debug Mode"})

FishingTab:Toggle({
    Title = "Blatant Fishing (Debug Mode)",
    Desc = "Fishing with real-time debug display",
    Value = false,
    Callback = function(value)
        if value then
            StartBlatantFishingDebug()
        else
            IsBlatantFishing = false
            Remotes.RF_AutoFishing:InvokeServer(false)
            AddDebugLog("🛑 Debug fishing stopped", Color3.fromRGB(255, 100, 100))
        end
    end,
})
